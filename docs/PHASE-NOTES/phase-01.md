...
## Temperature — a deeper look

### What's actually happening under the hood

When an LLM generates a response, it doesn't "think" and write a sentence. It predicts **one token at a time**. After each token, it produces a probability distribution over its entire vocabulary — every possible next token gets a score.
...
- **Temperature = 1.0** → use the distribution as-is. The model picks tokens proportional to their probability. "blue" wins most of the time, but "beautiful" or "grey" can occasionally win too.

- **Temperature < 1.0** → the distribution gets **sharpened**. High-probability tokens get even higher, low-probability tokens get crushed toward zero. At `0.0` it's completely deterministic — always picks the highest probability token, no randomness at all.

- **Temperature > 1.0** → the distribution gets **flattened**. Tokens that were unlikely become more competitive. The model takes more risks. Outputs get weird. Most providers cap at 1.0 or 2.0.
...
| Range | Use case | Why |
|---|---|---|
| `0.0` | SQL generation, JSON extraction, code, structured output | You want the same correct answer every time |
| `0.1–0.3` | Question answering, summarization, factual tasks | Focused but not completely rigid |
| `0.4–0.7` | Explanations, writing assistance, chat | Natural variation without going off-rails |
| `0.8–1.0` | Brainstorming, creative writing, generating options | Variety is the point |
...
People assume higher temperature = smarter or more creative. That's wrong.

Higher temperature = **more random**. It doesn't make the model think harder — it makes it deviate more from what's most likely. On a hard reasoning task, temperature 1.0 will make it *worse* because it introduces randomness into token choices that should be deterministic.

For most production tasks, **0.0–0.3 is the right default**. You reach for higher temperature specifically when variety is what you want — not as a way to improve quality.

---

### One more thing: top-p (nucleus sampling)

Temperature has a sibling called `top_p`. Instead of scaling the whole distribution, it cuts off the tail — "only consider tokens that together account for the top P% of probability mass."

- `top_p=1.0` → consider all tokens (default, no cutoff)
- `top_p=0.9` → only consider the tokens that make up the top 90% of probability

In practice: **don't tune both at the same time**. Pick one. Most teams either fix temperature and leave `top_p=1.0`, or fix `top_p` and leave `temperature=1.0`. Adjusting both simultaneously makes it very hard to reason about what's causing output changes.
